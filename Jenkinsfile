    ...
    stages {
        ...
        stage('Build image | Angular | Dev') {
            ...
            steps {
                ...
                sh 'curl -v -X POST -H "Content-Type:application/tar" --data-binary "@KwetterAngular.tar.gz" http://192.168.1.11:2375/build?t=ma.ade/kwetterangular:latest&buildargs=%7B%22NG_PROJECT_NAME%22%3A%22Kwetter-Angular%22%2C%22ENVIRN%22%3A%22stage%22%7D'
            }
        }
		stage('Build image | Angular | Master') {
            ...
            steps {
                sh 'tar -cvf KwetterAngular.tar.gz -C Kwetter-Angular .'
                sh 'curl -v -X POST -H "Content-Type:application/tar" --data-binary "@KwetterAngular.tar.gz" http://192.168.1.11:2375/build?t=ma.ade/kwetterangular:latest&buildargs=%7B%22NG_PROJECT_NAME%22%3A%22Kwetter-Angular%22%2C%22ENVIRN%22%3A%22prod%22%7D'
            }
        }
